...
async function readAllProducts(type,billingbranch) {
    console.log(type);
    ...
    html += '<div class="grid-item col-sm-6">';
        ...
        if (!data.image) {
            ...
            html += '<div class="card-body">';
                html += '<div title=\"'+data.productname+'"\ class="description">'+ data.productname  +'</div>';
...
function getService (data) {
    ...
    html += '<div class="grid-item col-sm-6">';
        ...
        if (!data.image) {
            ...
            html += '<div style="background-color:#C4BFC2;"><img class="card-img-top cover" src="../images/productimage/'+ data.image +'" alt="product image"></div>';
            html += '<div class="card-body">';
                html += '<div title=\"'+data.servicename+'"\ class="description">'+ data.servicename  +'</div>';
                html += '<div class="d-flex">';
                html += '<div class="card-text mr-2 mb-2 price-'+data.id+'">₱'+ formatNumber(data.selling_price) +'</div>';
                var style = "";
                if (data.qty == 0) {
...
function searchUser()
        {
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    $('#input-patientname').val(data[0].name);
...
function on_Click () {
    ...
    $(document).on('click', '#btn-add-to-tray',  function(){
        ...
        console.log(qty)
        if (stock < total_qty && type == 'Product') {
    ...
    $(document).on('click', '#proccess', function(){
        ...
        var id = $('#input-search-userid').val();
        let billingbranch2 = $('#billingbranch').val()
        let wholesale_discount_amount2 = localStorage.getItem('wholesale_discount_amount');
        let senior_pwd_discount_amount2 = localStorage.getItem('senior_pwd_discount_amount');
                ...
                if (invoice_no) {      
                    ...
                    $.ajax({
                        ...
                        data: {
                            ...
                            id             : id,
                            billingbranch2 : billingbranch2,
                            wholesale_discount_amount2 : wholesale_discount_amount2,
                            senior_pwd_discount_amount2 : senior_pwd_discount_amount2
                        },
                        success:async function(res){
                            ...
                            if (res == 'success') {
                                ...
                                let type = $('#type').val()
                                let billingbranch = $('#billingbranch').val()
                                let patientname = $('#input-patientname').val()
                                console.log(type);
                                ...
                                {
                                    window.open("/preview-invoice/"+wholesale_discount_amount +"/"+senior_pwd_discount_amount+"/"+billingbranch+"/"+patientname);
                                    setTimeout(async function()
                                    {
                                        await readTray();
                                    },300);
    ...
    $('#type').change(function()
      {
         ...
         console.log(type);
         console.log(billingbranch);
         readAllProducts(type,billingbranch);
      });

      $('#billingbranch').change(function()
      {
         let type = $('#type').val()
         let billingbranch = $('#billingbranch').val()
         console.log(type);
