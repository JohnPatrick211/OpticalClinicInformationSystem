$(document).ready(function(){
    $.ajaxSetup({
      ...
      function load_data(){
        let patienthistorybranch = $('#patienthistorybranch').val()
        fetchPatientHistory(patienthistorybranch);
      }

      function fetchPatientHistory(patienthistorybranch){

      var tbl_for_validation = $('#datatable-adminpatienthistory').DataTable({

          processing: true,
          serverSide: true,

          ajax:{
           url: "adminpatienthistory-data",
           data:{
            patienthistorybranch:patienthistorybranch,
           },
          },
   ...
   $('#select-all').on('click', function(){
    ...
    $('#for-validation-table tbody').on('change', 'input[type="checkbox"]', function(){
      ...
      $('#patienthistorybranch').change(function()
      {
        let patienthistorybranch = $('#patienthistorybranch').val()
        $('#datatable-adminpatienthistory').DataTable().destroy();
        fetchPatientHistory(patienthistorybranch);
      });
      ...
      function fetchHistoryInfo(invoice_no){
        $.ajax({
          url:"/adminpatienthistory/getpatienthistoryinfo/"+ invoice_no,
          type:"GET",
          ...
          success:function(data){
            ...
            window.open("adminpatienthistory/preview-invoice/"+wholesale_discount_amount +"/"+senior_pwd_discount_amount+"/"+billingbranch+"/"+patientname+"/"+invoice_no);
          }
