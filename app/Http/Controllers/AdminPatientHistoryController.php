...
class AdminPatientHistoryController extends Controller
{
    public function HistoryView(){
        if(Session::has('LoggedUser')){
            ...
            $users4 = ClinicBranch::orderBy('id', 'ASC')->get();
             ...
             return view('admin-patient-history', $data)->with('users4',$users4);


         }
    }

    public function PatientHistoryData(Request $request)
    {
        $getEm = $this->getHistory($request->patienthistorybranch);
         if(request()->ajax())
             {
    ...
    public function getHistory($patienthistorybranch)
    {
        if($patienthistorybranch == "All Branches")
        {
            ...
            ->select('tbl_sales.*', 'tbl_branch.branchname')
            ->leftJoin('tbl_branch', 'tbl_sales.branch_id', '=', 'tbl_branch.id')
            ->where('branch_id',$patienthistorybranch)
            ->groupBy('tbl_sales.invoice_no')
            ->get();
